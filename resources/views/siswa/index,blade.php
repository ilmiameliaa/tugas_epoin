...
<body>
  ...
  <table class="tabel">
    ...
    <tr>
      <td>
        <img scr="{{ asset('storage/siswas/'.$siswa->image) }}" width="120px" hight="120px" alt="">
      </td>
      <td>{{ $siswa->nis }}</td>
      <td>{{ $siswa->name}}</td>
      <td>{{ $siswa->email}}</td>
      ...
      @endif
      <td>
        <form onsubmit="return confirm('Apakah Anda Yakin?')" ; action="{{ route('siswa.destroy', $siswa->id)}} method="
          POST">
